        ...
        <div class="card mb-8 box-shadow">
          ...
          <div class="card-header" style="background-color:  white;">
             <div class="media text-muted pt-3" style="direction:  rtl;">
                <img src="{{asset('images/avatar/'.$post->user->avatar)}}" alt="{{$post->user->name}}" class="col-sm-2
                 rounded" style="margin-right: -3%; width: 50px;height: 50px;">
                <div class="media-body pb-3 mb-0" style="text-align: right;direction:  rtl;" >
                 ...
                 <div style="text-align: left;direction:  ltr;">
                     <button class="btn btn-sm btn-outline-secondary">
                         <a class="badge badge-light" href="{{route('post.edit', $post->id)}}" >edit</a>
                     </button>

                 </div>
                 @endcan
                 @can('delete' ,$post)
                  <form action="{{route('post.destroy', $post->id)}}" method="POST" id="ajax_unlike">
                    {{ csrf_field() }}
                    {{ method_field('DELETE') }}

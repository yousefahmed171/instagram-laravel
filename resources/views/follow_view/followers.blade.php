                    ...
                    <div class="my-3 p-3 bg-white rounded box-shadow" style="direction:  rtl;text-align:  right;">
                        ...
                        @foreach ($follow_requests as $request)
                            <div class="media text-muted pt-3">
                                <img src="{{asset('images/avatar/'.$request->from_user['avatar'])}}" alt=""
                                     class="col-sm-2 mr-2 rounded" style="margin-right: -3%;width: 50px;height: 50px;">
                ...
                <div class="col-md-6">
                    <div class="my-3 p-3 bg-white rounded box-shadow" style="direction:  rtl;text-align:  right;">
                        ...
                        @foreach ($followers as $follower)
                            ...
                            @endphp
                            <div class="media text-muted pt-3">
                                <img src="{{asset('images/avatar/'.$user['avatar'])}}" alt="" class="col-sm-2 mr-2
                                rounded"
                                ...
                                <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
                                    <div class="d-flex justify-content-between align-items-center w-100">
                                        ...
                                        <form method="post" action="{{route('follow.destroy', $follower->id)}}">
                                            {{ csrf_field() }}
                                            {{method_field('DELETE')}}
                                            <input type="submit" name="redirect_to" class="btn btn-outline-danger"
                                                   value="{{$follower->from_user['id'] == auth()->user()->id ? 'إلغاء المتابعة' : 'حذفه من المتابعين' }}">
                                        </form>
                                    </div>
